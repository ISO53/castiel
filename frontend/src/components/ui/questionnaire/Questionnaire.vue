...
const props = defineProps({
  ...
  defaultItem: { type: String, required: false },
  item: { type: String, required: false },
  items: { type: Array, required: false },
  noValidate: { type: Boolean, required: false, default: true },
  shortcuts: { type: null, required: false },
});

const emits = defineEmits(["reset", "submit", "update:item"]);

const rootElement = ref(null);
const registrations = shallowRef([]);
const domVersion = ref(0);
const pendingFocus = shallowRef(null);

const collection = computed(() => createQuestionnaireCollection(props.items));
const uncontrolledItem = ref(
...
function setItem(nextItem, focusTarget = "item") {
...
function setItemAt(index, focusTarget = "item") {
