...
const answerControls = shallowRef([]);
  ...
  return new Map(
    shortcutAnswers
      ...
      .flatMap((answer, index) =>
        keys[index] ? [[answer.id, keys[index]]] : [],
      ),
  );
});
...
function updateAnswerSelected(answerId, selected) {
  if (!selected) {
...
function setAnswerSelectionFromInteraction(answerId, selected) {
  skipped.value = false;
  updateAnswerSelected(answerId, selected);
}

function syncControlledAnswerSelection(answerId, selected) {
  if (selected) {
    skipped.value = false;
  }
...
function registerAnswerSelection(answerId, defaultSelected) {
  if (defaultSelected) {
...
function setAnswerDefault(answerId, defaultSelected) {
  if (defaultSelected) {
...
function registerAnswerControl(registration) {
...
function registerDescription(descriptionId) {
...
function registerTitle(titleId) {
...
function registerError(errorId) {
...
function focusInvalid() {
  const selectedInput = itemElement.value?.querySelector(
    "input[data-filled][name]:not(:disabled)",
  );
  const firstControl = itemElement.value?.querySelector(
...
function getAnswerByElement(element) {
  return answers.value.find((answer) => answer.element === element) ?? null;
}

function getAnswerByShortcut(shortcut) {
  if (shortcutByChoiceValue.value) {
...
function moveAnswerFocus(currentElement, direction) {
...
<template>
  <fieldset
    ...
    tabindex="-1"
    :class="
      cn('flex flex-col gap-3 min-w-0 border-0 p-0 outline-none', props.class)
    "
  >
