#!/usr/bin/env bash
# Builds the castiel desktop bundle for Linux: a self-contained app-image
# (castiel launcher + private bundled runtime, no installer, no system Java)
# tarballed for direct use. Output lands in packaging/dist/.
#
# Usage:  ./package-linux.sh [--skip-ui-build] [--skip-maven] [--skip-archive]
set -euo pipefail
